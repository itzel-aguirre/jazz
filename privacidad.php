<?php
include 'header.php';
/**
 * 	@author Amilkhael Chávez Delgado;
 *	Documento: Index del la landing
 */
?>

<section class="container-fluid">
  <div class="row row--background-red justify-content-center">
    <div class="col-6">
      <h1 class="title title--marginleft-less text-center">Aviso de privacidad</h1>
    </div>
  </div>
  <div class="container">
    <p class="paragraph-privacy">
      DOS CON TODO, S.A. DE C.V., con domicilio ubicado en Calle Milán Número 14
      Planta Baja, Colonia Juárez, Delegación Cuauhtémoc, Código Postal 06600,
      Ciudad de México, correo electrónico hola@parker-lenox.com, será el
      responsable de los Datos Personales que sean proporcionados por las personas
      físicas y/o morales a quien correspondan los mismos, para ello pone a su
      disposición este Aviso de Privacidad en el que garantiza a los mismos el uso
      adecuado y limitado de la información personal que sea intercambiada entre
      los titulares de los datos y nosotros, los mismos serán protegidos conforme
      a lo dispuesto en los artículos 11, 12, 13, 14, 15, 16, 17, 19 y 20 de la
      Ley Federal de Protección de Datos Personales en Posesión de los
      Particulares y los relativos a su reglamento hace de su conocimiento.
    </p>
    <p class="paragraph-privacy">
      DOS CON TODO, S.A. DE C.V., hace de su conocimiento que los datos personales
      que usted voluntariamente nos proporciona, serán tratados de manera
      confidencial, debidamente conservados y resguardados bajo un estricto orden
      de seguridad, con la única finalidad de llevar a cabo la gestión y
      actividades enfocadas al cumplimiento de las obligaciones pasadas presentes
      o futuras derivadas de las relaciones jurídicas o comerciales que se
      establezcan entre el titular de los datos personales y DOS CON TODO, S.A. DE
      C.V., mismos que serán usados con fines de mercadotecnia, publicitarios o de
      prospección comercial, conservando dicha información personal por el tiempo
      que sea necesario para dar cumplimiento a las finalidades ya precisadas con
      antelación, o bien, las señaladas en el Aviso de Privacidad Integral de esta
      empresa.
    </p>
    <p class="paragraph-privacy">
      DOS CON TODO, S.A. DE C.V., entenderá que el titular ha manifestado su
      consentimiento, si este, no manifiesta la revocación a través del
      procedimiento referido en el Aviso de Privacidad Integral, que puede ser
      consultado en los portales electrónicos de esta empresa.
    </p>
    <p class="paragraph-privacy">
      Para mayor información de los términos y condiciones, favor de consultar el
      Aviso de Privacidad Integral o bien de manera física dentro de las
      instalaciones por el personal autorizado.
    </p>
  </div>
  
 

</section>

<?php include 'footer.php'; ?>
